...
// Get total count
$stmt = $pdo->prepare('SELECT COUNT(*) as count FROM contact_enquiries WHERE trip_type = "trekking"');
$stmt->execute();
...
<html lang="en">
  <head>
    ...
    <title>Trek Quotations and Customer Info | Rudraansh Tours & Travel</title>
  ...
  <body data-page="quotations">
     ...
     <?php include 'sidebar.php'; ?>
      ...
      <section class="content-area">
        <header class="top-banner">
          <p>Quotation Requests and Customer Information</p>
          <span>Total quotations: <?php echo $totalCount; ?></span>
        </header>
        ...
        <section class="panel-card">
          ...
          <div class="leads-count">
            Showing <?php echo count($leads) > 0 ? ($offset + 1) : 0; ?> to <?php echo min($offset + count($leads), $totalCount); ?> of <?php echo $totalCount; ?> quotations
          </div>
          ...
          <?php else: ?>
            <div class="empty-state">
              <p>No quotation requests found in database.</p>
    ...
    <script>
      ...
      function deleteLead(id, name) {
        if (confirm(`Are you sure you want to delete the quotation for ${name}? This action cannot be undone.`)) {
          fetch('../backend/delete-quotation.php', {
            method: 'POST',
            headers: {
